...
class TestCreatingUser:
    ...
    def test_creating_an_existing_user(self, create_and_delete_user):
        ...
        responce = requests.post(f"{burgers_url}/auth/register", json=payload)
        responce_json=responce.json()
        assert responce.status_code == 403
        assert responce_json.get("message") == "User already exists"
    ...
    def test_creating_a_user_without_required_fields(self):
        ...
        responce_json = responce.json()
        assert responce.status_code == 403
        assert responce_json.get("message") == "Email, password and name are required fields"
    ...
    def test_creating_a_user_without_email(self, create_user_without_name):
    ...
    def test_creating_a_user_without_email(self, create_user_without_password):
